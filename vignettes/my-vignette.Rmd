...
binom.rf2 <- SMARTVIS::binomialRF(X,factor(y), fdr.threshold = .05,
                     ntrees = 10000,percent_features = .8,
                     fdr.method = 'BY')

binom.rf3 <- SMARTVIS::binomialRF(X,factor(y), fdr.threshold = .05,
                     ntrees = 10000,percent_features = .5,
                     fdr.method = 'BY')

binom.rf4 <- SMARTVIS::binomialRF(X,factor(y), fdr.threshold = .05,
                     ntrees = 10000,percent_features = .3,
                     fdr.method = 'BY')
...
We recommend growing at least 5-10k trees so that the algorithm has a chance to stabilize. Simulation studies show so far that ntrees ~ 10,000 yields satisfactory results. The ntrees tuning parameter must be set in conjunction with the percent_features as these two are inter-connected. 


```{r echo=F}
binom.rf <- SMARTVIS::binomialRF(X,factor(y), fdr.threshold = .05,
                     ntrees = 1000,percent_features = .2,
                     fdr.method = 'BY')

binom.rf2 <- SMARTVIS::binomialRF(X,factor(y), fdr.threshold = .05,
                     ntrees = 5000,percent_features = .2,
                     fdr.method = 'BY')

binom.rf3 <- SMARTVIS::binomialRF(X,factor(y), fdr.threshold = .05,
                     ntrees = 10000,percent_features = .2,
                     fdr.method = 'BY')

binom.rf4 <- SMARTVIS::binomialRF(X,factor(y), fdr.threshold = .05,
                     ntrees = 50000,percent_features = .2,
                     fdr.method = 'BY')
...
$X1 = \begin{bmatrix} X_1& X_4 & X_6 & X_9 \end{bmatrix}$ 

$X2 = \begin{bmatrix} X_1 & X_2 &  X_3 & X_4 & X_6  &X_9 \end{bmatrix}$ 

$X3 = \begin{bmatrix} X_3 & X_4 & X_5 & X_6 & X_7  &X_8 &X_9 &X_{10} \end{bmatrix}$ 

$X4 = \begin{bmatrix} X_5 & X_6 & X_7  &X_8 &X_9 \end{bmatrix}$ 

$X5 = \begin{bmatrix} X_4 & X_5  &X_8 &X_{10} \end{bmatrix}$ 

$X6 = \begin{bmatrix} X_1 & X_2 &  X_3 & X_4 & X_5 & X_6 & X_7  &X_8 &X_9 &X_{10} \end{bmatrix}$ 
...
require(data.table)
 ...
 a= SMARTVIS::evaluateCandidateModels(candidateModels, X,y,percent_features = .4, ntrees = 10000)
